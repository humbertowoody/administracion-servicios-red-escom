...
def findNetworkID(ip, con):
  output = con.send_command(f'show ip interface brief | i {ip}')
  net = output.split()
  output = con.send_command(f'sh run | i {net[1]}')
  mask = output.split()
...
def rip(con):
  ...
  con.send_command('configure terminal\n')
  time.sleep(1)
  con.send_command('no ip routing\n')
  time.sleep(5)
  con.send_command('ip routing\n')
  time.sleep(2)
  con.send_command('router rip\n')
  time.sleep(1)
  con.send_command('version 2\n')
  time.sleep(1)
  con.send_command('no auto-summary\n')
  time.sleep(2)

  for i in range(len(ip_id)):
    print('RIP Network '+ip_id[i]["idRed"])
    con.send_command('network '+ip_id[i]["idRed"]+'\n')
    time.sleep(1)

  output = con.send_command('exit\n')
  time.sleep(1)
  output = con.send_command('exit\n')
  time.sleep(1)
...
def ospf(con):
  ...
  con.send_command('ip routing')
  time.sleep(2)
  con.send_command('router ospf 1')
  time.sleep(1)

  for i in ip_id:
    comando_ospf = 'net '+i["idRed"]+" "+i["wildcard"]+" area 0"
    print('COMANDO OSPF:', comando_ospf)
    con.send_command(comando_ospf)
    time.sleep(1)

  con.send_command('exit')
  time.sleep(2)
  con.send_command('exit')
  time.sleep(2)
  return 1
